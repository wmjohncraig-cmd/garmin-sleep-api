...
### 8. Coaching State (replaced Audit v2.0 on 2026-03-03)
```
Coaching chat → POST /coaching-state → JSONBin → GET /coaching-state → Dashboard brief
```
- **Storage**: JSONBin bin `JSONBIN_COACHING_STATE_BIN_ID` (`69a76e9943b1c97be9b099c0`)
- **Not AI-generated**: Updated manually by coaching chat, not auto-generated by Claude API
- **Read path**: `GET /coaching-state` → dashboard brief displays COACHING PLAN section
- **Write path**: `POST /coaching-state` (full replace) or `POST /coaching-state?merge=1` (partial merge)
- **Contents**: race_calendar, this_week (day-by-day prescription), active_injuries, key_decisions, race_targets, swim_reference, proactive_triggers
- **Server cache**: In-memory, 2-min TTL
...
- **Old audit**: `/coaching-audit` endpoint and `AUDIT_V2_SYSTEM_PROMPT` removed entirely
...
| `JSONBIN_COACHING_STATE_BIN_ID` | Coaching state document (replaces audit) |
...
| ~~`imtx_audit_cache`~~ | removed | Audit replaced by coaching state |
